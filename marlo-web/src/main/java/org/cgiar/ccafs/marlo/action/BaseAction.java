...
public class BaseAction extends ActionSupport implements Preparable, SessionAware, ServletRequestAware {
  ...
  public Boolean getAutoSaveFilePath(String simpleName, String actionName, long id) {
    ...
    String autoSaveFile = id + "_" + composedClassName + this.getActualPhase().getDescription() + "_"
  ...
  public Phase getCenterCrpPhase(GlobalUnit crp) {
    ...
    try {
      ...
      if (this.getSession().containsKey(APConstants.CENTER_CRP_PHASE)) {
        ...
        Phase phase = phaseManager.findCycle(cycle, year, crp.getId());
  ...
  public Phase getCurrentCenterPhase() {
    ...
    try {
      ...
      if (this.getSession().containsKey(APConstants.CENTER_CURRENT_PHASE)) {
        ...
        Phase phase = phaseManager.findCycle(APConstants.REPORTING, this.getActualPhase().getYear(), this.getCrpID());
  ...
  public Boolean hasDeliverableRule(DeliverableInfo deliverableInfo, String rule) {
    if (deliverableInfo != null && deliverableInfo.getDeliverableType() != null) {
      List<DeliverableTypeRule> deliverableTypeRules = deliverableInfo.getDeliverableType().getDeliverableTypeRules()
        .stream().filter(dtr -> dtr.getDeliverableRule().getName().equals(rule)).collect(Collectors.toList());
      if (deliverableInfo.getDeliverableType().getDeliverableCategory() != null) {
        deliverableTypeRules
          .addAll(deliverableInfo.getDeliverableType().getDeliverableCategory().getDeliverableTypeRules().stream()
